...
inline void configuration::standard(PartitionConfig &partition_config) {
    ...
    partition_config.restream_amount = 1;
    partition_config.mode = STRONG;

    partition_config.cpm_gamma = 0.5;
    partition_config.clusters_to_ix_mapping.resize(0);
    partition_config.score = 0;
    
    partition_config.ext_clustering_algorithm = NO_EXT_ALGORITHM;
    partition_config.cluster_fraction = 1.0;
    partition_config.ext_algorithm_time = 300;
    partition_config.ls_time_limit = 600;
    partition_config.ls_frac_time = 0.5;
    partition_config.max_num_clusters = -1;
    partition_config.max_cluster_edge_volume = std::numeric_limits<unsigned int>::max();

    partition_config.strong_cut_off = 0.1;
    partition_config.cut_off = 0.05;
    partition_config.offset_interval = 10;
    partition_config.partialOffsets = NULL;
    
    partition_config.activeNodes_set = NULL;
}
